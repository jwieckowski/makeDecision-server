...
def test_results_calculation_crisp_mcda_method_ranking_correlation(client):
    ...
    assert payload['response'][7]['node_type'] == 'visualization'

def test_results_calculation_crisp_weights_correlation(client):
    """
    ...
    assert type(payload['response'][3]['data'][0]) is dict
    assert payload['response'][3]['node_type'] == 'visualization'

def test_results_calculation_crisp_method_ranking_visualization(client):
    """
        Test verifying the functionality of multi-criteria calculations for basic model structure with weights methods with crisp data and visualization of ranking visualization
    """
    data = [
        {
            "id": 1,
    ...
    assert type(payload['response'][5]['data'][0]) is dict
    assert payload['response'][5]['node_type'] == 'visualization'
